...
if ($item != "") {
    $sqlSelect = "SELECT * FROM mutasi_part_bekas WHERE Nama_Barang LIKE '%$item%'";
} else {
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $sheet->setCellValue("A$rowIndex", $row['Area Penyimpanan']) // Pastikan nama kolom sesuai
              ...
              ->setCellValue("J$rowIndex", $row['Keterangan_Potensi_Pemakaian_Barang']);
        $rowIndex++;
    }
}
...
// Membuat nama file
if ($item != "") {
    $fileName = "Data $item Bekas ($tanggalexport).xlsx";
} else {
    $fileName = "Data Barang Bekas ($tanggalexport).xlsx";
}
